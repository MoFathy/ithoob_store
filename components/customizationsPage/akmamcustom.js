...
// import $ from "jquery";
export class AkmamCustom extends Component {
componentDidMount(){
            ...
            akmamObject.map((item,index)=>{
              return (
                <div className="custom__items" key={"akmam"+item.id} style={this.props.itemid !== undefined ? {display:"block"} : index !== 0 ? {display:"none"} : {} }>
                  <AkmamContent type="akmamArray"  Id={item.id}  itemid={this.props.itemid}   customObject={item} key={"akmam"+item.id} />
                </div>
              )
            })
          }
...
export default connect(mapStateToProps, mapDispatchToProps)(AkmamCustom);
