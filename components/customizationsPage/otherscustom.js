...
// import $ from "jquery";
export class OthersCustom extends Component {
  componentDidMount(){
            ...
            othersObject.map((item,index)=>{
              return(
                <div className="custom__items"  key={"other"+item.id} style={this.props.itemid !== undefined ? {display:"block"} : index === 0 || item.info ? {display:"block"} : {display:"none"} }>
                  <OthersContent  type="othersArray" Id={item.id}  itemid={this.props.itemid}  customObject={item} key={"other"+item.id} />
                </div>
              )
            })
          }
...
export default connect(mapStateToProps, mapDispatchToProps)(OthersCustom);
