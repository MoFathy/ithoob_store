...
export class FabricsCustom extends Component {
            ...
            fabricsObject.map((item,index)=>{
              return (
                <div className="custom__items" key={"fabric"+item.id} style={this.props.itemid !== undefined ? {display:"block"} : index !== 0 ? {display:"none"} : {} }>
                  <FabricContent type="fabricArray"  itemid={this.props.itemid} Id={item.id} customObject={item} key={"fabric"+item.id}/>
                </div>
              )
            })
          }
...
export default connect(mapStateToProps, null)(FabricsCustom);
