...
import YakaContent from '../../components/includes/newCarousel';
import { connect } from 'react-redux';
import { getYaka } from '../../actions/customizationsPage/yakaActions';
// import $ from "jquery";
export class YakaCustom extends Component {
componentDidMount(){
            ...
            yakaObject.map((item,index)=>{
              return (
                <div className="custom__items" key={"yaka"+item.id} style={this.props.itemid !== undefined ? {display:"block"} : index !== 0 ? {display:"none"} : {} }>
                  <YakaContent type="yakaArray"  Id={item.id}  itemid={this.props.itemid}  customObject={item} key={"yaka"+item.id} />
                </div>
            )
          })
          }
...
export default connect(mapStateToProps, mapDispatchToProps)(YakaCustom);
